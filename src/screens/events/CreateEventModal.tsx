// src/screens/events/CreateEventModal.tsx
...
export const CreateEventModal = ({ navigation }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      navigation.replace('EVENT_CREATED', { eventId: event.id, eventTitle: event.title });
    }
  };
  ...
  return (
    <div className="flex-1 bg-obsidian flex flex-col h-full">
      <div className="p-6 border-b border-white/5 flex items-center justify-between bg-obsidian/50 backdrop-blur-xl">
        ...
        <h2 className="text-xl font-bold text-alabaster tracking-tight">Create Event</h2>
        <div className="w-10" />
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-8">
        ...
        <Button type="submit" className="w-full py-5 text-xl shimmer mt-10" disabled={uploading || !photo}>Publish Event</Button>
      </form>
    </div>
  );
};
